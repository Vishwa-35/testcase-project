document.addEventListener("DOMContentLoaded", function () {
    ...
    if (pieEl) {
        ...
        const numericValues = statusValues.map(function(val) {
            const num = parseFloat(val);
            return isNaN(num) ? 0 : num;
        });

        const hasData = numericValues.some(function(val) {
            return val > 0;
        });

        if (statusLabels.length > 0 && hasData) {
            try {
                ...
                pieEl.parentElement.innerHTML = '<p class="text-muted text-center p-3">No data available</p>';
            }
        } else {
            pieEl.parentElement.innerHTML = '<p class="text-muted text-center p-3">No data available</p>';
        }
